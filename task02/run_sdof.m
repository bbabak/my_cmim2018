...
% if solver == 45
    ...
    [T1,Y1]=ode45(@(t,y) sdof(F,m,k,omega,c,t,y,analysis,solver),tspan,y0,options);
% elseif solver == 15
    display('Integrator: ode15s (Matlab), options:')
    [T2,Y2]=ode15s(@(t,y) sdof(F,m,k,omega,c,t,y,analysis,solver),tspan,y0,options);
% elseif solver == 23
    display('Integrator: ode23s (Matlab), options:')
    [T3,Y3]=ode23s(@(t,y) sdof(F,m,k,omega,c,t,y,analysis,solver),tspan,y0,options);
% elseif solver == 24
    display('Integrator: ode23t (Matlab), options:')
    [T4,Y4]=ode23t(@(t,y) sdof(F,m,k,omega,c,t,y,analysis,solver),tspan,y0,options);
% end
toc
